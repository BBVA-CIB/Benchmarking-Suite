 ...
 *
 */
public class TestPerformanceMeasures implements IAction<Boolean>
{
	@Override
	public ActionResult<Boolean> execute (TestContext testContext, JobContext jobContext)
	{
		
		jobContext.getUserMeasures().addStartMeasure("ownMeasure", jobContext.getJobIteration(), System.nanoTime());
		long x = 1;
		long y = 1;
		...
		res.setResult(y<x);
		jobContext.getUserMeasures().addEndMeasure("ownMeasure", jobContext.getJobIteration(), System.nanoTime());
		return res;
	}
}
